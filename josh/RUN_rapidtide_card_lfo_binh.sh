#!/bin/bash
# bash RUN_rapidtide_card_lfo_binh.sh
# sbatch --cpus-per-task=16 --mem=64g --time 60:00:00 RUN_rapidtide_card_lfo_binh.sh
...
#for sbjid in "15"; do
    ...
    card_lfo_data=/data/SFIM_physio/physio/physio_results/sub${sbjid}/sub${sbjid}_lfo_downsampled2TR_arr_inhold.tsv
    out_prefix=sub${sbjid}_inhold_card_lfo_delay_1030
    out_dir=/data/SFIM_physio/data/derivatives/sub${sbjid}/rapidtide/cardiac_lfo8
    cd ${out_dir}

    # when encountered the binary operator expected warning, changed below line to double brackets, which didn't work
    if [ -f ${fmri_data} ]; then
