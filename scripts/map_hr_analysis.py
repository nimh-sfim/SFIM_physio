...
# import sys
# sys.path.insert(1, '/Users/deanjn/Documents/GitHub/afni/src/python_scripts/afnipy')
# #sys.path.append('/Users/deanjn/Documents/GitHub/afni/src/python_scripts/afnipy')
# import lib_physio_peaks
...
#### applying Paul's function to avoid shift! ####
# hr_data_smooth, int_idx = module.apply_bandpass_smooth(hr_data, 1, 0, max_bps=None, label='', retobj=None, win_shape='blackman_nuttall', verb=0)
#### Don't forget to apply this to the MAP as well! ####
...
upper_thres_scalar = 1
lower_thres_scalar = 1
...
plt.subplot(5, 1, 3); plt.plot(hr_time, hr_map_ratio); plt.ylabel('HR/MAP \nRatio')
plt.subplot(5, 1, 4); plt.plot(hr_time, z_hr_map_ratio); plt.ylabel('Norm \nHR/MAP \nRatio')
...
plt.legend() 
plt.subplot(5, 1, 5); plt.plot(hr_time, binarized_regr); plt.ylabel('Task \nRegressor')
plt.xlabel('Time (seconds)')   #indices are defined by position in between peaks

plt.savefig('/Volumes/SFIM_physio/physio/' + subj_id + '/' + subj_id + '_QC_bin_regr.png')

plt.show()

## Save the binarized task regressor: 1 positive, 1 negative

np.savetxt('/Volumes/SFIM_physio/physio/' + subj_id + '/' + subj_id + '_bin_regr.tsv', binarized_regr, '%.4f', delimiter = '\t')
